...
**Capability basis:** CAP-019 Provider gateway

[↑ 000 Governed Turn](./000-GOVERNED-TURN.md) · [← SSR & Context Assembly](./SSR-CONTEXT-ASSEMBLY.md) · [→ Tool Workcell & Proof](./TOOL-WORKCELL-PROOF.md)

## Scope

Defines the public-safe process boundary, responsibility flow, material handoffs, decisions and degraded/failure behavior for **Forklift & Inference Dispatch** in the V5 runtime. Current production supplies the exercised parity pattern; V5 reconstructs that responsibility behind explicit contracts and approved hardening. Production and V5 evidence remain separately labeled.

## Inputs

Context Receipt + provider plan + authorized tool schemas.

## Outputs

Model Result / tool proposal / explicit route failure.

## Process / decision logic

1. Pure route planner selects a known/enabled route.
2. Decision: capability compatible?
3. Decision: data boundary preserved?
4. Invoke through provider adapter / model gateway.
5. Decision: invocation success?
6. If failed: fallback explicitly allowed?
7. If allowed: fallback preserves same constraints? If yes retry through valid route; otherwise fail closed/degrade explicitly.
8. Return Model Result with invocation provenance and optional tool proposal.

## Governing control / applicable controlled documents

This process is governed for public documentation by **CTRL-500**. The control family states what is governed. Exact private thresholds/policy expressions remain withheld.
